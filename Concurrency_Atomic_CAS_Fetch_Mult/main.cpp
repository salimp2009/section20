...
T fetch_mult(std::atomic<T>& shared, T mult) {
    ...
    return oldValue;                                                   // returns with the old value
}
...
int main()
{
    ...
    std::cout<<fetch_mult(myInt, 25)<<'\n';
    std::cout<<myInt<<'\n';
    
    std::thread t1(fetch_mult<int>, std::ref(myInt), 10);       // need to pass type info for fethc_mult
